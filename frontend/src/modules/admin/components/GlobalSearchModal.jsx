...
export const GlobalSearchModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-3 sm:p-6 sm:pt-20 bg-slate-900/60 backdrop-blur-xs">
      <motion.div
        initial={{ opacity: 0, scale: 0.96, y: -10 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.96, y: -10 }}
        transition={{ duration: 0.15 }}
        className="w-full max-w-xl bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden flex flex-col max-h-[85vh]"
      >
        {/* Top Input Header */}
        <div className="p-3.5 sm:p-4 border-b border-slate-200/90 flex items-center gap-2.5">
          ...
          <input
            ...
            className="flex-1 text-sm font-normal text-slate-900 placeholder:text-slate-400 focus:outline-hidden"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
          ...
          <span className="hidden sm:inline-block px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-[10px] font-mono font-medium text-slate-500">
            ESC
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
          {loading ? (
            ...
            <div className="py-8 text-center space-y-1">
              <p className="text-xs font-semibold text-slate-700">Quick Store Search</p>
                    ...
                    {customers.map((c) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="w-8 h-8 rounded-md bg-slate-800 text-white flex items-center justify-center text-xs font-medium shrink-0">
        ...
        {/* Footer shortcuts */}
        <div className="px-4 py-2.5 bg-slate-50/80 border-t border-slate-200/80 flex items-center justify-between text-xs text-slate-400">
